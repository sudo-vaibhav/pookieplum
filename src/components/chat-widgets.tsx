...
export function StickerWidgetComponent({ widget }: { widget: StickerWidget }) {
  return (
    <div className="p-3 bg-gradient-to-br from-white via-gray-50 to-gray-100 rounded-2xl shadow-lg border border-gray-200 animate-slide-up hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      ...
      <p className="text-xs text-center mt-1 text-gray-600 animate-fade-in font-medium">
        {widget.stickerName}
      </p>
    </div>
  );
  ...
  return (
    <div className="p-5 bg-gradient-to-br from-green-100 via-emerald-50 to-green-50 border-2 border-green-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-2 hover:rotate-1 relative overflow-hidden">
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <span className="font-bold text-green-800 text-shadow-sm">
          Money Sent
        </span>
      </div>
      ...
      <div className="text-3xl font-black text-green-900 mb-2 animate-bounce-in drop-shadow-lg">
        {widget.currency}{" "}
        {widget.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
      </div>

      {widget.note && (
        <p className="text-sm text-green-700 animate-fade-in bg-green-200 bg-opacity-50 p-2 rounded-lg border border-green-300 italic">
          {widget.note}
        </p>
      )}
...
export function DaysSinceWidgetComponent({
  widget,
}: {
  widget: DaysSinceWidget;
}) {
  ...
  return (
    <div className="p-5 bg-gradient-to-br from-purple-100 via-pink-100 to-purple-100 border-2 border-purple-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-2 hover:-rotate-1 relative overflow-hidden">
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <span className="font-bold text-purple-800 text-shadow-sm">
          Days Since
        </span>
      </div>

      <div className="text-center">
        <div className="text-4xl mb-2 animate-bounce drop-shadow-lg">
          {widget.emoji || "📅"}
        </div>
        ...
        <p className="text-sm text-purple-700 font-bold animate-fade-in bg-purple-200 bg-opacity-60 p-2 rounded-lg border border-purple-300">
          {widget.eventName}
        </p>
...
export function SharedLocationWidgetComponent({
  widget,
}: {
  widget: SharedLocationWidget;
}) {
  return (
    <div className="p-4 bg-gradient-to-br from-blue-100 via-cyan-50 to-blue-100 border-2 border-blue-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-1 relative overflow-hidden">
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <span className="font-bold text-blue-800 text-shadow-sm">
          Location Shared
        </span>
      </div>

      <div className="space-y-2">
        <h3 className="text-lg font-black text-blue-900 drop-shadow-sm">
          {widget.locationName}
        </h3>
...
export function DateReminderWidgetComponent({
  widget,
}: {
  widget: DateReminderWidget;
}) {
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-orange-100 via-amber-50 to-orange-100 border-2 border-orange-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-1 relative overflow-hidden">
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <span className="font-bold text-orange-800 text-shadow-sm">
          Date Reminder
        </span>
      </div>
      ...
      <div className="text-center mb-3">
        ...
        <h3 className="text-lg font-black text-orange-900 drop-shadow-sm mb-1">
          {widget.eventName}
        </h3>
...
export function CalendarEventWidgetComponent({
  widget,
}: {
  widget: CalendarEventWidget;
}) {
  const eventDate = new Date(`${widget.eventDate}T${widget.eventTime}`);
  ...
  return (
    <div
      className={`p-4 bg-gradient-to-br from-${color}-100 via-${color}-50 to-${color}-100 border-2 border-${color}-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-1 relative overflow-hidden`}
    >
      {/* Skeumorphic shine effects */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-white to-transparent opacity-70"></div>
      <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-white to-transparent opacity-50"></div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-2">
          <div
            className={`w-8 h-8 bg-gradient-to-br from-${color}-400 via-${color}-500 to-${color}-600 rounded-full flex items-center justify-center shadow-inner border-2 border-${color}-300 relative`}
          >
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white to-transparent opacity-40"></div>
            <Calendar className="h-4 w-4 text-white drop-shadow-sm relative z-10" />
          </div>
          <span
            className={`font-semibold text-${color}-800 text-shadow-sm text-sm`}
          >
            Event
          </span>
        </div>
        <div
          className={`text-xs px-2 py-1 bg-${color}-200 text-${color}-800 rounded-full font-bold`}
        >
      ...
      <div className="space-y-2">
        <h3 className={`text-lg font-black text-${color}-900 drop-shadow-sm`}>
          {widget.eventName}
        </h3>

        <div
          className={`text-sm font-bold text-${color}-700 bg-${color}-200 bg-opacity-60 p-2 rounded-lg border border-${color}-300`}
        >
          {eventDate.toLocaleDateString("en-IN", {
            weekday: "short",
            month: "short",
        ...
        {widget.location && (
          <div
            className={`text-xs text-${color}-600 flex items-center space-x-1`}
          >
            <MapPin className="h-3 w-3" />
            <span>{widget.location}</span>
          </div>
        ...
        {widget.description && (
          <p
            className={`text-xs text-${color}-600 italic bg-white bg-opacity-50 p-2 rounded`}
          >
            {widget.description}
          </p>
        )}
      ...
      {/* Bottom depth shadow */}
      <div
        className={`absolute bottom-0 left-0 w-full h-2 bg-gradient-to-t from-${color}-900 to-transparent opacity-15 rounded-b-2xl`}
      ></div>
    </div>
  );
}

export function GoneLiveWidgetComponent({
  widget,
}: {
  widget: GoneLiveWidget;
}) {
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-red-100 via-pink-100 to-red-100 border-2 border-red-200 rounded-2xl max-w-xs shadow-2xl hover:shadow-3xl transition-all duration-500 animate-slide-up transform hover:-translate-y-1 relative overflow-hidden">
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <span className="font-bold text-red-800 text-shadow-sm">
          Gone Live!
        </span>
        {widget.isActive && (
      ...
      <div className="space-y-3">
        <div className="text-center">
          ...
          <h3 className="text-lg font-black text-red-900 drop-shadow-sm mb-1">
            {widget.streamTitle}
          </h3>
